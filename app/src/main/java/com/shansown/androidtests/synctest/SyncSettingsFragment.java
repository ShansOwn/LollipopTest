...
public class SyncSettingsFragment extends PreferenceFragment
  ...
  @Override public void onCreate(Bundle savedInstanceState) {
    ...
    final ListPreference interval =
        (ListPreference) getPreferenceManager().findPreference(PrefUtils.PREF_CUR_SYNC_INTERVAL);
  ...
  @Override public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
    ...
    switch (key) {
      ...
      case PrefUtils.PREF_CUR_SYNC_INTERVAL:
        final ListPreference interval = (ListPreference) getPreferenceManager().findPreference(
            PrefUtils.PREF_CUR_SYNC_INTERVAL);
        interval.setSummary(interval.getEntry());

                /*final long interval = Long.parseLong(
                        ((ListPreference) getPreferenceManager().findPreference(key)).getValue());*/
        SyncHelper.updateSyncInterval(context, account, Long.parseLong(interval.getValue()), true);
        break;
    }
  }
}