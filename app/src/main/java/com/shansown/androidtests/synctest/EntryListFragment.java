...
public class EntryListFragment extends Fragment
  ...
  private static final String[] PROJECTION = new String[] {
      AppContract.Entry._ID, AppContract.Entry.ENTRY_ID,
      AppContract.Entry.ENTRY_TITLE, AppContract.Entry.ENTRY_LINK,
      AppContract.Entry.ENTRY_PUBLISHED
  };
  ...
  private static final String[] FROM_COLUMNS = new String[] {
      AppContract.Entry.ENTRY_TITLE, AppContract.Entry.ENTRY_PUBLISHED
  };
  ...
  @Override public View onCreateView(LayoutInflater inflater, ViewGroup container,
    ...
    mAdapter.setViewBinder((view1, cursor, i) -> {
      if (i == COLUMN_PUBLISHED) {
        // Convert timestamp to human-readable date
        Time t = new Time();
        t.set(cursor.getLong(i));
        ((TextView) view1).setText(t.format("%Y-%m-%d %H:%M"));
        return true;
      } else {
  ...
  @Override public Loader<Cursor> onCreateLoader(int id, Bundle args) {
    ...
    return new CursorLoader(getActivity(),  // Context
        AppContract.Entry.CONTENT_URI, // URI
        PROJECTION,                // Projection
        null,                           // Selection
        null,                           // Selection args
        AppContract.Entry.ENTRY_PUBLISHED + " desc"); // Sort
  }
  ...
  @Override public void onStatusChanged(int which) {
    ...
    final boolean syncActive =
        ContentResolver.isSyncActive(account, AppContract.CONTENT_AUTHORITY);
    final boolean syncPending =
        ContentResolver.isSyncPending(account, AppContract.CONTENT_AUTHORITY);
