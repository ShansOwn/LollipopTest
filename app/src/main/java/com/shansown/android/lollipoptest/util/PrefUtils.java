...
private static final String TAG = makeLogTag(PrefUtils.class);
    ...
    public static void setCurSyncInterval(final Context context, long interval, boolean sync) {
        ...
        SharedPreferences.Editor editor = sp.edit()
                .putString(PREF_CUR_SYNC_INTERVAL, String.valueOf(interval));
        if (sync) {
            editor.commit();
        } else {
            editor.apply();
    ...
    public static void setAccountSetupComplete(final Context context, boolean complete, boolean sync) {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
        SharedPreferences.Editor editor = sp.edit()
                .putBoolean(PREF_ACCOUNT_SETUP_COMPLETE, complete);
        if (sync) {
            editor.commit();
        } else {
            editor.apply();
